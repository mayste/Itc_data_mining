...
class CompanyPageScraper(Scraper):
    ...
    def catch_company_data(self, company):
        ...
        company.set_company_industry(
            self.catch_optional_text_value_by_xpath(self.config['Path']['COMPANY_INDUSTRY_XPATH']))
        # Catch company type
        ...
        if company_type is not None and self.config['Constant']['DASH'] in company_type:
            company_type = company_type.split(self.config['Constant']['DASH'])[
                int(self.config['Constant']['SECOND_ELEMENT'])].strip()
            company.set_company_type(company_type)
        else:
            company.set_company_type(company_type)

        # Catch company revenue
        company.set_company_revenue(
            self.catch_optional_text_value_by_xpath(self.config['Path']['COMPANY_REVENUE_XPATH']))
        # Catch company headquarters
        company.set_company_headquarters(
            self.catch_optional_text_value_by_xpath(self.config['Path']['COMPANY_HEADQUARTER_XPATH']))
